...
async function open(viewport, save = "") {
  ...
  await page.getByRole("button", { name: /PLAY NOW|CONTINUE RUN/ }).waitFor();
  return page;
}
...
const partialSave = JSON.stringify({ version: 2, coins: 1250, upgrades: [], tutorialComplete: true, bestScore: 4400, settings: { music: true, sfx: true, reducedMotion: false }, activeRun: null });
...
await page.getByRole("button", { name: "Build ticket item 3" }).click();
